          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <div className="space-y-2">
                <Label>Company</Label>
                <Select><SelectTrigger><SelectValue placeholder="All Companies" /></SelectTrigger>
                  <SelectContent><SelectItem value="all">All Companies</SelectItem>{mockCompanies.map((c) => (<SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>))}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label>Shop</Label>
                <Select><SelectTrigger><SelectValue placeholder="All Shops" /></SelectTrigger>
                  <SelectContent><SelectItem value="all">All Shops</SelectItem>{mockShops.map((s) => (<SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>))}</SelectContent>
                </Select>
              </div>
            </div>
          </CardContent>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" className="gap-2" onClick={() => handleDownload('pdf')}><Download className="h-4 w-4" />Download PDF</Button>
                <Button variant="outline" size="sm" className="gap-2" onClick={() => handleDownload('excel')}><Download className="h-4 w-4" />Download Excel</Button>
              </div>
            </div>
          </CardHeader>
          <CardContent>
                  ...
                  <TableBody>
                    {mockPurchases.map((p) => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell className="text-right text-destructive">₹{p.netPayable.toLocaleString()}</TableCell>
                      </TableRow>
                    ))}
                  ...
                  <TableBody>
                    {mockSales.map((s) => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell>{new Date(s.saleDate).toLocaleDateString('en-IN')}</TableCell>
                        <TableCell>{s.items.length} items</TableCell>
                        <TableCell className="text-right text-success">₹{s.netPayable.toLocaleString()}</TableCell>
                        <TableCell className="text-right text-primary">₹{s.profit.toLocaleString()}</TableCell>
                      </TableRow>
                    ))}
                    <TableRow className="border-t-2 font-bold">
                      <TableCell colSpan={4}>Total Sales & Profit</TableCell>
